  ...
  return (
    <div className="bg-[#fcfcfc] min-h-screen py-6 sm:py-8 border-b border-stone-200" id="catalog-page-container">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-5 border-b border-stone-200/80 mb-6">
          <div>
            <h1 className="font-serif-title text-2xl sm:text-3xl lg:text-4xl font-bold text-stone-900 tracking-tight">
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <button
              ...
              className="w-full sm:w-auto px-4 py-2.5 bg-stone-900 text-white rounded-xl text-xs sm:text-sm flex items-center justify-center gap-2 font-medium shadow-xs hover:bg-[#d4af37] hover:text-black transition"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <aside
            className={`lg:col-span-3 ${
              mobileFilterOpen
                ? 'fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex justify-end lg:static lg:z-auto lg:bg-transparent'
                : 'hidden lg:block'
            }`}
            id="catalog-filters-sidebar"
          >
            <div
              className={`w-full max-w-xs sm:max-w-sm lg:max-w-none h-full lg:h-auto overflow-y-auto lg:overflow-visible bg-white p-5 sm:p-6 lg:rounded-2xl border border-stone-200/90 shadow-sm space-y-6 ${
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-5">
                {paginatedProducts.map((product: Product) => {
                  ...
                  return (
                    <div
                      key={product.id}
                      className="bg-white rounded-2xl border border-stone-200/80 hover:border-stone-300 overflow-hidden shadow-xs hover:shadow-md transition-all duration-300 flex flex-col group relative"
                      id={`product-card-${product.sku}`}
                    >
                      {/* Imagen del Producto en la parte superior */}
                      <div
                        onClick={() => navigateToProduct(product)}
                        className="relative w-full aspect-square bg-stone-100 overflow-hidden cursor-pointer select-none"
                      >
                        <img
                        ...
                        <div className="absolute top-2.5 left-2.5 flex flex-col gap-1 z-10">
                          {isOutOfStock && (
                            <span className="px-2 py-0.5 bg-stone-900/95 text-white text-[9px] sm:text-[10px] font-bold rounded-md uppercase tracking-wider shadow-xs">
                              Agotado
                            </span>
                          )}
                          {isLowStock && !isOutOfStock && (
                            <span className="px-2 py-0.5 bg-amber-600 text-white text-[9px] sm:text-[10px] font-semibold rounded-md uppercase tracking-wider shadow-xs">
                              Últimas {product.stock}
                            </span>
                          )}
                          {product.isNew && !isOutOfStock && !isLowStock && (
                            <span className="px-2 py-0.5 bg-[#d4af37] text-stone-950 text-[9px] sm:text-[10px] font-bold rounded-md uppercase tracking-wider shadow-xs">
                              Nuevo
                            </span>
                          )}
                        </div>
                        ...
                        <button
                          ...
                          className="absolute top-2.5 right-2.5 w-8 h-8 rounded-full bg-white/90 backdrop-blur-xs shadow-xs flex items-center justify-center text-stone-600 hover:text-rose-500 transition-colors z-10 cursor-pointer"
                      ...
                      <div className="p-3 sm:p-3.5 flex-1 flex flex-col justify-between gap-2">
                        ...
                        <div>
                          ...
                          <h3
                            onClick={() => navigateToProduct(product)}
                            className="font-serif-title text-xs sm:text-sm font-bold text-stone-900 group-hover:text-[#b58d24] transition-colors cursor-pointer line-clamp-2 leading-snug min-h-[2rem]"
                            title={product.name}
                          >
                            {product.name}
                          </h3>
                        ...
                        <div className="pt-2.5 border-t border-stone-100 flex flex-col gap-2">
                          ...
                          <div className="flex items-baseline justify-between">
                            <div className="flex items-baseline gap-1">
                              <span className="font-serif-title text-sm sm:text-base font-bold text-stone-950 tracking-tight">
                          ...
                          <button
                            ...
                            className={`w-full py-2 px-3 rounded-xl text-xs font-bold tracking-wide flex items-center justify-center gap-1.5 transition-colors cursor-pointer ${
                              isOutOfStock
