...
export const AdminInventoryView: React.FC<AdminInventoryViewProps> = ({ controller }) => {
  ...
  return (
    <div className="p-4 sm:p-8 space-y-6 bg-[#fcfcfc] min-h-screen text-stone-800">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-stone-500 text-xs sm:text-sm mt-0.5">
            Administra el catálogo completo de Mujer Latina.
          </p>
        </div>
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-2.5">
          ...
          <div className="relative flex-1 sm:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-3.5 py-2 bg-white border border-stone-200 rounded-xl text-xs sm:text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-[#d4af37] focus:ring-2 focus:ring-[#d4af37]/20 shadow-2xs transition"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          <div className="relative">
            <button
              ...
              className={`px-3.5 py-2 bg-white border rounded-xl text-xs sm:text-sm font-medium transition flex items-center gap-2 shadow-2xs ${
                activeFiltersCount > 0
                  ? 'border-[#d4af37] text-stone-900 bg-[#d4af37]/5 font-semibold'
                  : 'border-stone-200 text-stone-700 hover:bg-stone-50'
              }`}
            >
              <SlidersHorizontal size={14} className={activeFiltersCount > 0 ? 'text-[#d4af37]' : 'text-stone-500'} />
              <span>Filtros</span>
              {activeFiltersCount > 0 && (
                <span className="w-4 h-4 rounded-full bg-[#d4af37] text-stone-950 font-bold text-[10px] flex items-center justify-center">
                  {activeFiltersCount}
                </span>
              )}
            </button>
              ...
              <div className="absolute right-0 mt-2 w-64 bg-white border border-stone-200 rounded-2xl shadow-xl p-4 z-40 space-y-3.5 animate-in fade-in zoom-in-95 duration-100">
                ...
                <div className="pt-2 border-t border-stone-100 flex justify-end">
                  <button
                    ...
                    className="px-3 py-1 bg-stone-900 hover:bg-black text-white text-xs font-semibold rounded-lg transition"
                  >
                    Aplicar
                  </button>
                </div>
          ...
          <button
            ...
            className="px-4 py-2 bg-black hover:bg-stone-900 text-[#d4af37] font-semibold rounded-xl text-xs sm:text-sm transition flex items-center gap-1.5 shadow-sm border border-stone-800 flex-shrink-0"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-stone-200 shadow-2xs flex items-center gap-4">
          ...
          <div>
            ...
            <p className="font-serif-title text-2xl font-bold text-stone-900 leading-tight">
              {totalProductsCount}
            </p>
          </div>
        </div>
        ...
        {/* Card 3: Agotados */}
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-stone-200 shadow-2xs flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-rose-50 text-rose-600 border border-rose-100 flex items-center justify-center flex-shrink-0">
            <AlertCircle size={22} />
          </div>
          <div>
            <p className="text-xs text-stone-500 font-medium">
      ...
      <div className="bg-white rounded-2xl border border-stone-200 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs sm:text-sm">
            ...
            <thead className="bg-stone-50 border-b border-stone-200 text-stone-500 font-semibold uppercase tracking-wider text-[11px]">
              <tr>
                <th className="py-3.5 px-5">IMAGEN</th>
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-stone-50/70 transition">
                      ...
                      <td className="py-3 px-5">
                        <div className="w-11 h-11 rounded-lg bg-stone-100 overflow-hidden border border-stone-200 flex-shrink-0 flex items-center justify-center">
                          {p.imageUrl ? (
                            <img
                              src={p.imageUrl}
                              alt={p.name}
                      ...
                      {/* SKU */}
                      <td className="py-3 px-4 font-mono font-medium text-xs text-stone-700 whitespace-nowrap">
                        {p.sku}
                      </td>
                      ...
                      <td className="py-3 px-5 text-right whitespace-nowrap">
                        <div className="inline-flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            className="border border-stone-200 hover:border-stone-300 hover:bg-stone-50 rounded-lg px-2.5 py-1 text-xs text-stone-700 font-medium flex items-center gap-1.5 transition shadow-2xs"
                            title="Editar información del producto"
                          >
                            <Pencil size={13} className="text-stone-600" />
                            <span>Editar</span>
                          </button>
                          ...
                          <button
                            ...
                            className="border border-stone-200 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 rounded-lg px-2.5 py-1 text-xs text-stone-600 font-medium flex items-center gap-1.5 transition shadow-2xs"
        ...
        <div className="px-5 py-3.5 border-t border-stone-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-stone-500 bg-stone-50/40">
          <div>
            Mostrando{' '}
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="w-7 h-7 rounded-lg border border-stone-200 flex items-center justify-center hover:bg-white disabled:opacity-30 disabled:pointer-events-none transition"
      ...
      {deletingProductId && productToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl border border-stone-200 space-y-4 animate-in fade-in zoom-in-95 duration-100">
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-stone-100">
              <button
                ...
                className="px-4 py-2 border border-stone-200 rounded-xl text-xs font-semibold text-stone-700 hover:bg-stone-50 transition"
              >
                Cancelar
              </button>
              <button
