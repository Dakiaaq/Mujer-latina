...
export const AdminCustomersView: React.FC<AdminCustomersViewProps> = ({ controller }) => {
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter((c) => {
      ...
      // Order count filter
      if (orderCountFilter === 'frequent' && c.ordersCount < 3) return false;
      if (orderCountFilter === 'occasional' && (c.ordersCount < 1 || c.ordersCount > 2)) return false;
      if (orderCountFilter === 'new' && c.ordersCount > 0) return false;
  ...
  }, [customers, search, selectedDept, orderCountFilter, sortBy]);
  ...
  return (
    <div className="bg-white min-h-screen text-stone-900 p-6 sm:p-10 space-y-6">
                ...
                currentItems.map((cust) => {
                  ...
                  return (
                    <tr key={cust.id} className="hover:bg-stone-50/70 transition">
                      ...
                      {/* Column 5: Pedidos (Badge) */}
                      <td className="py-4 px-6">
                        <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-stone-100 text-stone-800 border border-stone-200/80">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3.5 bg-stone-50 rounded-xl border border-stone-200">
                  <div className="flex items-center gap-1.5 text-stone-500 mb-1">
                    <ShoppingBag size={14} className="text-[#b58d24]" />
                    <span className="font-medium">Total Pedidos</span>
                  </div>
                  <p className="text-lg font-bold text-stone-900">
                    {viewingCustomer.ordersCount}
                  </p>
                </div>

                <div className="p-3.5 bg-stone-50 rounded-xl border border-stone-200">
                  <div className="flex items-center gap-1.5 text-stone-500 mb-1">
                    <DollarSign size={14} className="text-[#b58d24]" />
                    <span className="font-medium">Total Invertido</span>
                  </div>
                  <p className="text-lg font-bold text-[#b58d24] font-serif-title">
                    ${viewingCustomer.totalSpent.toLocaleString()} COP
                  </p>
                </div>
              </div>
